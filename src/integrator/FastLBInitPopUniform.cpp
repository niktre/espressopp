...
namespace espressopp {
   namespace integrator {
      //    LOG4ESPP_LOGGER(FastLBInitPopUniform::theLogger, "FastLBInitPopUniform");
      FastLBInitPopUniform::FastLBInitPopUniform(shared_ptr<System> system,
                                         shared_ptr< FastLB > fastlb)
      : FastLBInit(system, fastlb) {
      }

      void FastLBInitPopUniform::createDenVel (real _rho0, Real3D _u0) {
         if (mpiWorld->rank()==0) {
         ...
         real invCs2 = 1. / fastlb->getCs2();

         real invCs4 = invCs2*invCs2;
         real scalp, value;
         int _offset = fastlb->getHaloSkin();
         Int3D _Ni = fastlb->getMyNi();
         int _numVels = fastlb->getNumVels();	// number of velocities in the model
      ...
      void FastLBInitPopUniform::setForce (Real3D _force) {}
      void FastLBInitPopUniform::addForce (Real3D _force) {}

      void FastLBInitPopUniform::registerPython() {
         using namespace espressopp::python;

         class_<FastLBInitPopUniform, bases< FastLBInit > >
         ("integrator_FastLBInit_PopUniform",		init< shared_ptr< System >,
          shared_ptr< FastLB > >())
         .def("createDenVel", &FastLBInitPopUniform::createDenVel)
         ;
      }
   }
}
