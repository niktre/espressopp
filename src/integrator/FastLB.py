...
from _espressopp import integrator_FastLB

class FastLBLocal(ExtensionLocal, integrator_FastLB):
    def __init__(self, system, nodeGrid,
                 a = 1., tau = 1., numDims = 3, numVels = 19):
        if not (pmi._PMIComm and pmi._PMIComm.isActive()) or pmi._MPIcomm.rank in pmi._PMIComm.getMPIcpugroup():
            cxxinit(self, integrator_FastLB,
                    system, nodeGrid, a, tau, numDims, numVels)

if pmi.isController :
    class FastLB(Extension):
        __metaclass__ = pmi.Proxy
        pmiproxydefs = dict(
                            cls = 'espressopp.integrator.FastLBLocal',
