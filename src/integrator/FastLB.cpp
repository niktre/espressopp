...
namespace espressopp {
   ...
   namespace integrator {
      LOG4ESPP_LOGGER(FastLB::theLogger, "FastLB");

      /* LB Constructor; expects 1 Int3D, 2 reals and 2 integers */
      FastLB::FastLB(shared_ptr<System> _system, Int3D _nodeGrid,
      ...
      , numVels(_numVels)
      {
         ...
         FastLatticePar(_system, getNumVels(),getA(),getTau());
...
/*******************************************************************************************/
      ...
      void FastLB::connect() {
         _recalc2 = integrator->recalc2.connect ( boost::bind(&FastLB::zeroMDCMVel, this));
         _befIntV = integrator->befIntV.connect ( boost::bind(&FastLB::makeLBStep, this));
      }

/*******************************************************************************************/

      /* Setter and getter for the parallelisation things */
      void FastLB::setMyNeigh (int _dir, int _rank) { myNeigh[_dir] = _rank;}
      int FastLB::getMyNeigh (int _dir) { return myNeigh[_dir];}

      void FastLB::setNodeGrid (Int3D _nodeGrid) { nodeGrid = _nodeGrid;}
      Int3D FastLB::getNodeGrid () {return nodeGrid;}

      void FastLB::setMyPos (Int3D _myPos) { myPos = _myPos;}
      Int3D FastLB::getMyPos () { return myPos;}

      void FastLB::setHaloSkin (int _haloSkin) { haloSkin = _haloSkin;}
      int FastLB::getHaloSkin () {return haloSkin;}

      void FastLB::setMyNi (Int3D _myNi) { myNi = _myNi;}
      Int3D FastLB::getMyNi () {return myNi;}

      void FastLB::setMyLeft (Real3D _myLeft) { myLeft = _myLeft;}
      Real3D FastLB::getMyLeft () {return myLeft;}

      /* Setter and getter for the lattice model */
      void FastLB::setNi (Int3D _Ni) { Ni = _Ni;}
      Int3D FastLB::getNi () {return Ni;}

      void FastLB::setA (real _a) { a = _a;
         std::cout << "Lattice spacing (lu) " << a << std::endl;}
      real FastLB::getA () { return a;}

      void FastLB::setTau (real _tau) { tau = _tau;
         std::cout << "Lattice time step (lu) " << tau << std::endl;}
      real FastLB::getTau () { return tau;}

      void FastLB::setNumDims (int _numDims) { numDims = _numDims;
         std::cout << "Number of Dimensions " << numDims << std::endl;}
      int FastLB::getNumDims () { return numDims;}

      void FastLB::setNumVels (int _numVels) { numVels = _numVels;
         std::cout << "Number of Velocities " << numVels << std::endl;}
      int FastLB::getNumVels () { return numVels;}

      void FastLB::setEqWeight (int _l, real _value) { eqWeight[_l] = _value;}
      real FastLB::getEqWeight (int _l) {return eqWeight[_l];}

      void FastLB::setCi (int _l, Real3D _vec) {c_i[_l] = _vec;}
      Real3D FastLB::getCi (int _l) {return c_i[_l];}

      void FastLB::setCs2 (real _cs2) { cs2 = _cs2;}
      real FastLB::getCs2 () { return cs2;}

      void FastLB::setInvB (int _l, real _value) {inv_b[_l] = _value;}
      real FastLB::getInvB (int _l) {return inv_b[_l];}

      /* Setter and getting for LB viscosity control */
      void FastLB::setGamma (int _i, real _gamma) {gamma[_i] = _gamma;}
      real FastLB::getGamma (int _i) { return gamma[_i];}

      void FastLB::setGammaB (real _gamma_b) {setGamma(0, _gamma_b); initFluctuations();}
      real FastLB::getGammaB () { return getGamma(0);}

      void FastLB::setGammaS (real _gamma_s) {setGamma(1, _gamma_s); initFluctuations();}
      real FastLB::getGammaS () { return getGamma(1);}

      void FastLB::setGammaOdd (real _gamma_odd) {setGamma(2, _gamma_odd);}
      real FastLB::getGammaOdd () { return getGamma(2);}

      void FastLB::setGammaEven (real _gamma_even) {setGamma(3, _gamma_even);}
      real FastLB::getGammaEven () { return getGamma(3);}

      void FastLB::setViscB (real _visc_b) {visc_b = _visc_b;
      ...
      real FastLB::getViscB () { return visc_b;}

      void FastLB::setViscS (real _visc_s) {visc_s = _visc_s;
         real _fir = 2.*_visc_s;
         real _sec = getCs2() * getTau() * convTimeMDtoLB()/getNSteps() * convLenMDtoLB() / convMassMDtoLB();
         setGamma(1, (_fir - _sec) / (_fir + _sec));}
      real FastLB::getViscS () { return visc_s;}

      /* Setter and getter for LB-temperature control */
      void FastLB::setLBTemp (real _lbTemp) { lbTemp = _lbTemp; initFluctuations();}
      real FastLB::getLBTemp () { return lbTemp;}

      void FastLB::setDoFluct (bool _fluct) {fluct = _fluct;}
      bool FastLB::doFluct () {return fluct;}

      void FastLB::setPhi (int _l, real _value) {phi[_l] = _value;}
      real FastLB::getPhi (int _l) {return phi[_l];}

      /* Setter and getter for external and coupling force control */
      void FastLB::setDoExtForce (bool _extForce) {extForce = _extForce;}
      bool FastLB::doExtForce () {return extForce;}

      void FastLB::setDoCoupling (bool _coupling) {coupling = _coupling;}
      bool FastLB::doCoupling () {return coupling;}

      void FastLB::setExtForceLoc (Int3D _Ni, Real3D _extForceLoc) {
         return (*lbfor)[_Ni[0]][_Ni[1]][_Ni[2]].setExtForceLoc(_extForceLoc);   }
      Real3D FastLB::getExtForceLoc (Int3D _Ni) {
         return (*lbfor)[_Ni[0]][_Ni[1]][_Ni[2]].getExtForceLoc();   }
      void FastLB::addExtForceLoc (Int3D _Ni, Real3D _extForceLoc) {
         return (*lbfor)[_Ni[0]][_Ni[1]][_Ni[2]].addExtForceLoc(_extForceLoc);   }

      void FastLB::setFricCoeff (real _fricCoeff) { fricCoeff = _fricCoeff;}
      real FastLB::getFricCoeff () { return fricCoeff;}

      void FastLB::setNSteps (int _nSteps) { nSteps = _nSteps;}
      int FastLB::getNSteps () { return nSteps;}

      void FastLB::setPrevDumpStep (int _saveStep) { saveStep = _saveStep;}
      int FastLB::getPrevDumpStep () { return saveStep;}

      void FastLB::setTotNPart (int _totNPart) { totNPart = _totNPart;}
      int FastLB::getTotNPart () { return totNPart;}

      void FastLB::setFOnPart (int _id, Real3D _fOnPart) {fOnPart[_id] = _fOnPart;}
      Real3D FastLB::getFOnPart (int _id) {return fOnPart[_id];}
      void FastLB::addFOnPart (int _id, Real3D _fOnPart) {fOnPart[_id] += _fOnPart;}
      ...
      real FastLB::getLBMom (Int3D _Ni, int _l) {
         return (*lbmom)[_Ni[0]][_Ni[1]][_Ni[2]].getMom_i(_l);   }

      /* Helpers for MD to LB (and vice versa) unit conversion */
      real FastLB::convMassMDtoLB() {return 1.;}

      //#note: need a foolproof in case there is no access to the integrator (and getTimeStep) yet
      real FastLB::convTimeMDtoLB() {return 1. / (integrator->getTimeStep() * getTau());}
      real FastLB::convLenMDtoLB() {
         return getNi().getItem(0) / (getSystem()->bc->getBoxL().getItem(0) * getA());}

      /* Profiling definitions */
      void FastLB::setProfStep (int _profStep) { profStep = _profStep;}
      int FastLB::getProfStep () { return profStep;}

      /* Setter and getter for simulation parameters */
      void FastLB::setStepNum (int _step) { stepNum = _step;}
      int FastLB::getStepNum () { return stepNum;}

      void FastLB::setCopyTimestep (real _copyTimestep) { copyTimestep = _copyTimestep;}
      real FastLB::getCopyTimestep () { return copyTimestep;}

      void FastLB::setDoRestart (bool _restart) { restart = _restart;}
      bool FastLB::doRestart () { return restart;}

/*******************************************************************************************/

      /* INITIALIZATION OF THE LATTICE */
      void FastLB::initLatticeSize() {
...
/*******************************************************************************************/
      ...
      void FastLB::initLatticeModel () {
         ...
         for (int l = 0; l < getNumVels(); l++) {
            setEqWeight(l, FastLatticePar::getEqWeightLoc(l));
            setInvB(l, FastLatticePar::getInvBLoc(l));
         }
      }
...
/*******************************************************************************************/
      ...
      void FastLB::streaming(int _i, int _j, int _k) {
...
/*******************************************************************************************/
      ...
      void FastLB::calcRandForce (Particle& p) {
...
/*******************************************************************************************/
      ...
      void FastLB::calcViscForce (Particle& p) {
         int _offset = getHaloSkin();
         real _a = getA();
         real _invA = 1. / _a;
...
/*******************************************************************************************/
      ...
      void FastLB::galileanTransf (Real3D _specCmVel) {
         System& system = getSystemRef();
         CellList realCells = system.storage->getRealCells();
...
/*******************************************************************************************/
      ...
      void FastLB::readLBConf (int _mode) {
         if (_mode == 0) {
...
/*******************************************************************************************/
      ...
      void FastLB::copyForcesFromHalo () {
         int i, j, k, idx;             // running indices
...
/*******************************************************************************************/
      ...
      void FastLB::registerPython() {

         using namespace espressopp::python;
         class_<FastLB, shared_ptr<FastLB>, bases<Extension> >

         ("integrator_FastLB", init<	shared_ptr< System >, Int3D, real, real, int, int >())
         .add_property("nodeGrid", &FastLB::getNodeGrid, &FastLB::setNodeGrid)
         .add_property("a", &FastLB::getA, &FastLB::setA)
         .add_property("tau", &FastLB::getTau, &FastLB::setTau)
         .add_property("numDims", &FastLB::getNumDims, &FastLB::setNumDims)
         .add_property("numVels", &FastLB::getNumVels, &FastLB::setNumVels)
         .add_property("visc_b", &FastLB::getViscB, &FastLB::setViscB)
         .add_property("visc_s", &FastLB::getViscS, &FastLB::setViscS)
         .add_property("gamma_b", &FastLB::getGammaB, &FastLB::setGammaB)
         .add_property("gamma_s", &FastLB::getGammaS, &FastLB::setGammaS)
         .add_property("gamma_odd", &FastLB::getGammaOdd, &FastLB::setGammaOdd)
         .add_property("gamma_even", &FastLB::getGammaEven, &FastLB::setGammaEven)
         .add_property("lbTemp", &FastLB::getLBTemp, &FastLB::setLBTemp)
         .add_property("fricCoeff", &FastLB::getFricCoeff, &FastLB::setFricCoeff)
         .add_property("nSteps", &FastLB::getNSteps, &FastLB::setNSteps)
         .add_property("profStep", &FastLB::getProfStep, &FastLB::setProfStep)
         .add_property("getMyNi", &FastLB::getMyNi)
         .def("getLBMom", &FastLB::getLBMom)
         .def("setLBMom", &FastLB::setLBMom)
         .def("saveLBConf", &FastLB::saveLBConf)
         .def("keepLBDump", &FastLB::keepLBDump)
         .def("connect", &FastLB::connect)
         .def("disconnect", &FastLB::disconnect)
         ;
      }
   }
}
